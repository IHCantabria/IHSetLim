from IHSetLim import calibration, lim
...
model = calibration.cal_Lim(wrkDir+'/data/', depth=10, angleBathy=36, D50=0.427e-3, mf=0.02)
...
kr = best_model_run['parkr']
mu = best_model_run['parmu']
# S0 = best_model_run['parS0']

full_run = lim.lim(model.Hb, model.dt, model.A, model.mf, kr, mu, model.Sm, model.Obs[0])
# full_run = lim.lim(model.Hb, model.dt, model.A, model.mf, kr, mu, model.Sm, S0)
...
ax[0].plot(model.time, full_run, color='red',linestyle='solid', label= 'Lim et al.(2022)')
...
ax[1].plot(model.time, model.Eb/np.max(model.Eb),color='black',linestyle='solid', label='Best objf.='+str(bestobjf))
ax[1].set_ylim([0,1])
ax[1].set_xlim([model.time[0], model.time[-1]])
ax[1].set_yticks([0, 1], ['0', r'$E_{b max}$'])
plt.subplots_adjust(hspace=0.3)
fig.savefig('./results/Lim_Best_modelrun_'+str(config.cal_alg.values)+'.png',dpi=300)
